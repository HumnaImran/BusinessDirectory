...
class News extends Admin_Controller
{
    ...
    public function index($category_id = 0, $pagination_offset=0)
	{
        ...
        $config['total_rows'] = sw_count($this->data['news']);
        $config['per_page'] = 20;
    ...
    public function clone_post($post_id = NULL) 
        {
        ...
        if(empty($post_id)){
            $this->session->set_flashdata('error', 
                    lang_check('Clone not create, post missing'));
            redirect('admin/estate');
            exit();
        }
        /* fetch data */
        $post_data = $this->data['page'] = $this->page_m->get_lang($post_id, FALSE, $this->data['content_language_id']);
        if(empty($post_data)){
            $this->session->set_flashdata('error', 
                    lang_check('Clone not create, post missing'));
            redirect('admin/estate');
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            $repository_id_new = $this->repository_m->save(array('name'=>'page_m'));
            // Update page with new repository_id
            $data['repository_id'] = $repository_id_new;
